...
function Canvas() {
    ...
    function drawGrid() {
        ...
        for (let i = 0; i < window.innerWidth; i += gridStep) {
            grid.push(<Line points={[i, 0, i, window.innerHeight]} stroke="#ddd" strokeWidth={1} />);
        }

        for (let j = 0; j < window.innerHeight; j += gridStep) {
            grid.push(<Line points={[0, j, window.innerWidth, j]} stroke="#ddd" strokeWidth={1} />);
        }
