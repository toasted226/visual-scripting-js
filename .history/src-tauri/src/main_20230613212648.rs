...
fn enumerate_files(dir: &str) -> Result<Vec<String>, InvokeError> {
    ...
    let mut userfiles: Vec<UserFile> = Vec::new();
    ...
    for path in paths {
        ...
        let file_name = path_result.file_name().to_str().unwrap().to_string();
        let is_folder = path_result.path().is_dir();

        userfiles.push(UserFile { name: file_name, path: path_string, is_folder: is_folder });
    }
